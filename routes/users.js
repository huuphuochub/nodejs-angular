...
let full_user = {name:hassname,  password:hash,  date:date , role:role, namereal:username};
      ...
      if (err) {
          ...
          .then(match => {
            if (match) {
              ...
              res.json({ 'thongbao': true , id_user:arr[0].id_user,name:arr[0].namereal, token:token, role:arr[0].role});
          } else {
              // Mật khẩu không đúng
